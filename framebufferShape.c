...
int main(int argc, char** argv) 
{
	...
	for(int j =0; j < fbvar.yres_virtual; j++)
			for(int i =0; i<fbvar.xres_virtual; i++){
					int offset = (j+i *fbvar.xres_virtual + i) * NO_OF_COLOR;
	...
	for(int j =0; j < fbvar.yres; j++)
			for(int i =fbvar.xres_virtual* 2/3; i<fbvar.xres; i++){
...
pixel = makepixel(0,0,255);
	...
	offset = (150 + 150 * fbvar.xres_virtual) * NO_OF_COLOR;
    lseek(fd, offset, SEEK_SET);
    pixel = makepixel(255, 0, 0);    /* Red */
    write(fd, &pixel, NO_OF_COLOR);
    offset = (200 + 200 * fbvar.xres_virtual) * NO_OF_COLOR;
    lseek(fd, offset, SEEK_SET);
    pixel = makepixel(0, 255, 0);   /*  Green */
    write(fd, &pixel, NO_OF_COLOR);
    offset = (250 + 250 * fbvar.xres_virtual) * NO_OF_COLOR;
